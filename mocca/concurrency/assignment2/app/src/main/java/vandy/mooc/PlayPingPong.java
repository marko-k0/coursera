...
public class PlayPingPong implements Runnable {
    ...
    private static String TAG = "PlayPingPong";
     ...
     * HandlerThreads.
     */
    Handler ppHandlers[] = new Handler[2];
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            // Create the Handler that will service this type of Thread, i.e., either PING or PONG.
            ppHandlers[PingPong.PING.equals(mMyType) ? 0 : 1] = new Handler(this);
            ...
            if(mMyType.equals(PingPong.PING)) {
                Message msg = Message.obtain(ppHandlers[0]);
                msg.obj = ppHandlers[1];
                msg.sendToTarget();

                Log.d(TAG, "PING send to the target");
            }
        }
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            if (mIterationsCompleted < mMaxIterations) {
                ...
                mOutputStrategy.print(str);
            } else {
                // Shutdown the HandlerThread to the main PingPong thread can join with it.
                quit();
                return true;
            }
    ...
    public void run() {
        ...
        PingPongThread ppThreads[] = new PingPongThread[] {
        ...
        // Barrier synchronization to wait for all work to be done before exiting play().
        for (PingPongThread t : ppThreads) {
            try {
                t.join();
